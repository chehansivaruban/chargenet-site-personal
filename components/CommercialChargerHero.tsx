...
export default function CommercialChargerHero() {
  ...
  return (
    <div ref={containerRef} className="relative h-[95dvh] w-full bg-black z-10">
      <motion.section 
        style={{ scale, opacity }}
        className="sticky top-0 h-[95dvh] w-full overflow-hidden flex items-end rounded-b-[36px] md:rounded-b-[56px] shadow-[0_25px_60px_rgba(0,0,0,0.4)] bg-neutral-950 origin-bottom"
      >
        {/* Absolute Background Image representing a premium corporate setting */}
        <div className="absolute inset-0 z-0 select-none overflow-hidden">
          <motion.div style={{ scale: imgScale, y: imgY }} className="absolute inset-0 w-full h-full">
            <Image
              src="/commercial/hero-commercial.jpeg"
              alt="High-yield commercial charging infrastructure"
              fill
              priority
          ...
          {/* Aesthetic gradients matching high-end automotive configurations */}
          <div className="absolute inset-0 bg-gradient-to-t from-black via-black/45 to-transparent z-10" />
          <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-transparent to-transparent z-10" />
        </div>

        {/* Content with luxury entry transitions */}
        <div className="relative z-20 w-full max-w-7xl mx-auto px-6 md:px-12 pb-24 md:pb-32 flex flex-col justify-end">
          ...
          >
            ...
            <h1 className="font-display font-black text-4xl sm:text-5xl md:text-6xl lg:text-7xl tracking-tighter text-white uppercase leading-[0.92] select-none hover:text-[#357BCE] transition-colors duration-300">
              High-Capacity Commercial Power grids
            </h1>
            <p className="text-neutral-350 text-sm md:text-base font-light leading-relaxed max-w-2xl mt-6 lg:mt-8">
              Enable your location to generate active charging revenue, optimize electric service panel usage, and support multi-vehicle commercial fleets with chargeNET heavy-duty, zero-downtime platforms.
            </p>
          </motion.div>
        </div>
      </motion.section>
